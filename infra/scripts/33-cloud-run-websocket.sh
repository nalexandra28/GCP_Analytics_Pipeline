...
OTEL_COLLECTOR_URL="$(gcloud run services describe otel-collector \
  --region="${GCP_REGION}" \
  --project="${GCP_PROJECT_ID}" \
...
gcloud run deploy "${CLOUD_RUN_SERVICE}" \
  ...
  --min-instances=1 \
  --max-instances=1 \
  --set-env-vars="LOG_LEVEL=INFO,GCP_PROJECT_ID=${GCP_PROJECT_ID},EVENT_NOTIFICATIONS_SUB=${NOTIF_SUB},OTEL_EXPORTER_OTLP_METRICS_ENDPOINT=${OTEL_COLLECTOR_URL}/v1/metrics" \
  --project="${GCP_PROJECT_ID}"
